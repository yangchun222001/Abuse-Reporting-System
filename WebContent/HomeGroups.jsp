...
<div class="container">
    <div class="row">
        ...
        <div class="col-xs-6" style="padding:0px">
            <h3 class="text-right">
                <a id="btn-create" type="button" class="btn btn-primary" href="<%= response.encodeUrl(request.getContextPath() + "/Controller?action=newgrouphome") %>"><i class="glyphicon glyphicon-plus"></i>&nbsp;New Home Group</a>
            </h3> 
        </div>
      </div>
      <div class="row">
        ...
        <div id="no-more-tables" style="padding:3px"> 		               
            <table class="col-md-12 table-bordered table-striped table-condensed cf " style="padding:0px; width: 100%;">
        		...
        		<tbody class="searchable">
        			<%int i=0;
        			for(GroupHome groupHome: listOfGroupHome) {i++;%><tr >
        				<td data-title="Home Group ID"><%=groupHome.getId()%></td>
        				<td data-title="Status"><%=groupHome.getStatus() %></td>
        				<td data-title="Home Group Name"><%=groupHome.getMasterDataName()%></td>
        				<td data-title="Description"><%=groupHome.getDescription()%></td>
        				<td data-title="Address"><%=groupHome.getAddress()%></td>
						...
						<a id="button" class="btn btn-success btn-xs" href="<%= response.encodeUrl(request.getContextPath() + "/Controller?action=updategrouphome&groupHomeId="+groupHome.getId()) %>"><i class="glyphicon glyphicon-ok"></i>&nbsp;Update</a>
						</div>
						</td>
        			</tr><%} %>
	...
	<!-- Modal 1-->
    <%int j=0;
    for(GroupHome groupHome: listOfGroupHome){ j++;%>
<div class="modal fade" id="myModal<%=j %>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-dialog">
  <form method="post" action="<%= response.encodeUrl(request.getContextPath() + "/Controller?action=dodeletegrouphome") %>">
  <input type= "hidden" name="grouphomeid" value="<%=groupHome.getId()%>"></input>
    <div class="modal-content">
      ...
      <h4><span class="glyphicon glyphicon-exclamation-sign"></span>&nbsp;Are you sure you want to delete the Home Group: <b><%=groupHome.getMasterDataName()%></b> ?</h4>
      </div>
