...
<html lang="en">
  ...
  <!------------------------------------------- Nevigation Bar End-------------------------------------------------------------->
    ...
    <%List<DisabilityType> listOfDisabilityType = (List<DisabilityType>) request.getAttribute("listOfDisabilityType"); %>
    <%Disability disability = (Disability) request.getAttribute("disability"); %>
...
<div class="container">
    <div class="row">
      ...
      <div class="row">
      	  <form  name="frmForm" class="form-horizontal" method="post" action="<%= response.encodeUrl(request.getContextPath() + "/Controller?action=doupdatedisability") %>">
			<fieldset>
			<legend>Update Disability   <font color="red"> <c:out value="${message}" /></font></legend> 
      			<input type="hidden" name= "disabilityid" value ="<%=disability.getId() %>" />
      					<!-- Text input-->
				<div class="form-group">
				  <label class="col-md-4 control-label" for="disabilityname">Disability Name</label>  
				  <div class="col-md-4">
				  <input id="disabilityname" name="disabilityname" type="text" placeholder="disability name" value="<%=disability.getName() %>" class="form-control input-md" required>
				  </div>
				</div>
				
				<!-- Select Basic -->
				<div class="form-group">
				  <label class="col-md-4 control-label" for="disabilitytype">Disability Type</label>
				  <div class="col-md-4">
				    <select id="disabilitytype" name="disabilitytype" class="form-control">
				      <%for(DisabilityType disabilityType: listOfDisabilityType) {%><option value="<%=disabilityType.getId()%>" <%if(disability.getTypeId()==disabilityType.getId()) {%>selected<%} %> ><%=disabilityType.getType()%></option><%} %>
				    </select>
				  </div>
				</div>
				...
				<div class="form-group">
				  ...
				  <div class="col-md-4">                     
				    <textarea class="form-control" id="description" name="description" ><%=disability.getDescription() %></textarea>
				  </div>
				</div>
				...
				<div class="form-group">
				  ...
				  <div class="col-md-8">
				<a href="<%= response.encodeUrl(request.getContextPath() + "/Controller?action=disability") %>" id="cancel" name="cancel" class="btn btn-default">Cancel</a>
